...
import me.dinozoid.strife.module.implementations.exploit.DisablerModule;
...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            try {
                final PacketInboundEvent packetInboundEvent = new PacketInboundEvent(p_channelRead0_2_);
                DisablerModule.instance().directPacketRecieve(packetInboundEvent);
                Client.INSTANCE.getEventBus().post(packetInboundEvent);
                if (packetInboundEvent.isCancelled()) return;
                packetInboundEvent.getPacket().processPacket(this.packetListener);
