...
import me.dinozoid.strife.module.implementations.player.FastPlaceModule;
import me.dinozoid.strife.ui.GuiIngameHook;
import me.dinozoid.strife.ui.menu.StrifeMainMenu;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    private final TheAlteningAuthentication altening = TheAlteningAuthentication.mojang();

    public TheAlteningAuthentication altening() {
        return altening;
    }
...
//        this.drawSplashScreen(this.renderEngine);
        ...
        Client.INSTANCE.startup(false);

        if (this.serverName != null) {
            this.displayGuiScreen(new GuiConnecting(new StrifeMainMenu(), this, this.serverName, this.serverPort));
        } else {
            this.displayGuiScreen(new StrifeMainMenu());
        }
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.theWorld == null) {
            guiScreenIn = new StrifeMainMenu();
    ...
    public void clickMouse() {
        Client.INSTANCE.getEventBus().post(new MouseEvent(0));
    ...
    public void rightClickMouse() {
        Client.INSTANCE.getEventBus().post(new MouseEvent(1));
        if (!this.playerController.getIsHittingBlock()) {
            if (FastPlaceModule.instance().toggled()) {
                this.rightClickDelayTimer = FastPlaceModule.instance().delayProperty().getValue().intValue();
            } else {
    ...
    public void runTick() throws IOException {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            if (Keyboard.isKeyDown(Keyboard.KEY_PERIOD)) {
                this.displayGuiScreen(new GuiChat(Client.CHAT_PREFIX));
            }
            ...
            if (this.thePlayer.isUsingItem()) {
                ...
                if (this.gameSettings.keyBindAttack.isKeyDown()) {
                    if (this.objectMouseOver != null && this.objectMouseOver.typeOfHit.equals(MovingObjectPosition.MovingObjectType.BLOCK) && (!this.thePlayer.isSwingInProgress || this.thePlayer.swingProgressInt >= this.thePlayer.getArmSwingAnimationEnd() / 2 || this.thePlayer.swingProgressInt < 0)) {
                        this.thePlayer.swingProgressInt = -1;
                        this.thePlayer.isSwingInProgress = true;
                        this.effectRenderer.addBlockHitEffects(this.objectMouseOver.getBlockPos(), this.objectMouseOver.sideHit);
                    }
                }
    ...
    public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
        ...
        Client.INSTANCE.getEventBus().post(worldLoadEvent);
        System.gc();
        this.systemTime = 0L;
    }
