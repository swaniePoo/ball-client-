...
public class Gui
{
    ...
    public static void drawHorizontalLine(float startX, float endX, int y, int color)
    {
        if (endX < startX)
        {
            float i = startX;
    ...
    public static void drawTexturedModalRect(float xCoord, float yCoord, TextureAtlasSprite textureSprite, float widthIn, float heightIn)
    {
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
