...
import me.dinozoid.strife.module.implementations.visuals.BlurModule;
import me.dinozoid.strife.module.implementations.visuals.CustomCrosshairModule;
import me.dinozoid.strife.module.implementations.visuals.OverlayModule;
import me.dinozoid.strife.module.implementations.visuals.ScoreboardModule;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        if (scoreboardModule == null)
            scoreboardModule = ScoreboardModule.instance();
        if(overlayModule == null)
            overlayModule = OverlayModule.instance();
       ...
       // Client.INSTANCE.getEventBus().post(new Render2DEvent(partialTicks, scaledresolution));
        ...
        if (this.showCrosshair()) {
            int width = i / 2;
            int height = j / 2;
            CustomCrosshairModule crosshairModule = Client.INSTANCE.getModuleRepository().moduleBy(CustomCrosshairModule.class);
            if (crosshairModule.toggled()) {
                float thickness = crosshairModule.thicknessProperty().getValue().floatValue();
                boolean smooth = crosshairModule.smoothProperty().getValue();
                boolean outline = crosshairModule.outlineProperty().getValue();
                float w = crosshairModule.widthProperty().getValue().floatValue();
                float h = crosshairModule.heightProperty().getValue().floatValue();
                float outlineThickness = thickness - (thickness / 4);
                int color = OverlayModule.getColor(200);
                int outlineColor = Color.BLACK.getRGB();
                switch (crosshairModule.crosshairModeProperty().getValue()) {
                    case DEFAULT: {
                        float gap = crosshairModule.gapProperty().getValue().floatValue();
                        glPushMatrix();
                        if (smooth)
                            glEnable(GL_LINE_SMOOTH);
                        glDisable(GL_TEXTURE_2D);
                        if (outline) {
                            RenderUtil.drawHorizontalLine(width - gap + outlineThickness / 2, width - gap - outlineThickness / 2 - w, height, thickness + outlineThickness, outlineColor);
                            RenderUtil.drawHorizontalLine(width + gap - outlineThickness / 2, width + gap + outlineThickness / 2 + w, height, thickness + outlineThickness, outlineColor);
                            RenderUtil.drawVerticalLine(width, height - gap + outlineThickness / 2, height - gap - outlineThickness / 2 - h, thickness + outlineThickness, outlineColor);
                            RenderUtil.drawVerticalLine(width, height + gap - outlineThickness / 2, height + gap + outlineThickness / 2 + h, thickness + outlineThickness, outlineColor);
                        }
                        RenderUtil.drawHorizontalLine(width - gap, width - gap - w, height, thickness, color);
                        RenderUtil.drawHorizontalLine(width + gap, width + gap + w, height, thickness, color);
                        RenderUtil.drawVerticalLine(width, height - gap, height - gap - h, thickness, color);
                        RenderUtil.drawVerticalLine(width, height + gap, height + gap + h, thickness, color);
                        glColor4f(1, 1, 1, 1);
                    ...
                    case POINT: {
                        ...
                        glDisable(GL_TEXTURE_2D);
                        if (outline) {
                            glPointSize(thickness + outlineThickness);
                    ...
                    case CIRCLE: {
                        float radius = crosshairModule.radiusProperty().getValue().floatValue();
                        glPushMatrix();
                        glDisable(GL_TEXTURE_2D);
                        if (smooth) {
                            glEnable(GL_LINE_SMOOTH);
                        ...
                        if (outline) {
                            glLineWidth(thickness + outlineThickness);
                            glBegin(GL_LINE_LOOP);
                            RenderUtil.color(outlineColor);
                            for (int degrees = 0; degrees <= 360; degrees++) {
                                double radian = Math.toRadians(degrees);
                                glVertex2f(width + (float) (Math.cos(radian) * radius), height + (float) (Math.sin(radian) * radius));
                            }
                            glEnd();
                        }
                        glLineWidth(thickness);
                    ...
                    case SQUARE: {
                        if (outline) {
                            RenderUtil.drawRect(width - w / 2 - outlineThickness / 2, height - h / 2 - outlineThickness / 2, width + w / 2 + outlineThickness / 2, height + h / 2 + outlineThickness / 2, outlineColor);
                        }
                        RenderUtil.drawRect(width - w / 2, height - h / 2, width + w / 2, height + h / 2, color);
                        break;
                    }
                }
            } else {
                ...
                this.drawTexturedModalRect(width - 7, height - 7, 0, 0, 16, 16);
            }
        }
        ...
        if (scoreobjective1 != null) {
            if (scoreboardModule.visibleProperty().getValue() || !scoreboardModule.toggled()) {
                this.renderScoreboard(scoreobjective1, scaledresolution);
            }
        }

        final User user = Client.INSTANCE.getStrifeUser();
        final ScaledResolution sr = new ScaledResolution(mc);
        final String text = "\u00A77" + Client.RELEASE_BRANCH + " - \u00A7F" + Client.RELEASE_DATE + "\u00A7f - " + PlayerUtil.ircChatColor(user) + user.clientUsername();
        OverlayModule.drawStringWithShadow(text, sr.getScaledWidth() - OverlayModule.getWidth(text) - 2,
                sr.getScaledHeight() - (mc.currentScreen instanceof GuiChat ? 25 : OverlayModule.getHeight(text) - 2), -1);
        ...
        if(BlurModule.instance().toggled()) BlurModule.instance().blurShit();
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        float x = scoreboardModule.xPositionProperty().getValue().floatValue();
        float y = scoreboardModule.yPositionProperty().getValue().floatValue();

        int i1 = collection.size() * this.getFontRenderer().FONT_HEIGHT;
        scoreboardModule.height(i1 + this.getFontRenderer().FONT_HEIGHT);
        int j1 = scoreboardModule.toggled() ? (int) y + i1 + this.getFontRenderer().FONT_HEIGHT : scaledRes.getScaledHeight() / 2 + i1 / 3;
        int k1 = 3;
        int l1 = scoreboardModule.toggled() ? (int) x : scaledRes.getScaledWidth() - i - k1;
        int j = 0;
        ...
        for (Score score1 : collection) {
            ...
            int width = (scaledRes.getScaledWidth() - k1 + 2) - (scaledRes.getScaledWidth() - i - k1);
            int l = l1 + width;
            if (largestWidth < scaledRes.getScaledWidth() - width)
                largestWidth = scaledRes.getScaledWidth() - width;
            drawRect(l1 - 2, k, l, k + this.getFontRenderer().FONT_HEIGHT, 1342177280);
            if (j == collection.size() && Client.INSTANCE.getModuleRepository().moduleBy(StreamerModeModule.class).toggled()) {
                String[] split = s1.split(" ");
                if (split.length > 1) {
                    s1 = split[0] + " \u00A78\u00A7k " + ChatFormatting.stripFormatting(split[2]);
                }
            }
            if (j == 1) {
                RenderUtil.drawWaveString("strifeclient.club", l1, k);
            } else {
        ...
        if (mc.currentScreen instanceof GuiChat && scoreboardModule.toggled()) {
            if (scoreboardModule.hovered())
                RenderUtil.drawOutlinedRect(x - 2, y - 1, x + scoreboardModule.width(), y + scoreboardModule.height(), 2, new Color(0, 0, 0, 60).getRGB(), new Color(0, 0, 0, 160).getRGB());
            else
                RenderUtil.drawRect(x - 2, y - 1, x + scoreboardModule.width(), y + scoreboardModule.height(), new Color(0, 0, 0, 40).getRGB());
        }
        scoreboardModule.width(scaledRes.getScaledWidth() - largestWidth);
    }
    ...
    private void renderBossHealth() {
        if (!overlayModule.elementsProperty().selected(OverlayModule.Element.BOSSBAR) || !overlayModule.toggled())
            return;
        boolean selected = overlayModule.elementsProperty().selected(OverlayModule.Element.BOSSBAR) && overlayModule.toggled();
        String bossName;
        int statusBarTime;
        if(mc.currentScreen instanceof GuiChat && selected) {
            bossName = "\u00A7cStrife Client";
            statusBarTime = 999;
        } else {
            bossName = BossStatus.bossName;
            statusBarTime = BossStatus.statusBarTime;
        }
        if (bossName != null && statusBarTime > 0) {
            --statusBarTime;
            FontRenderer fontrenderer = this.mc.fontRendererObj;
            ScaledResolution scaledresolution = new ScaledResolution(this.mc);
            int i = scaledresolution.getScaledWidth();
            int width = 182;
            if (!overlayModule.isBossbarSet()) {
                overlayModule.setBossbarX(i / 2f - width / 2f);
                overlayModule.setBossbarY(12);
                overlayModule.setBossbarSet(true);
            }
            int x = selected ? (int) overlayModule.getBossbarX() : i / 2 - width / 2;
            int l = (int) ((mc.currentScreen instanceof GuiChat && selected ? 1 : BossStatus.healthScale) * (float) (width + 1));
            int y = selected ? (int) overlayModule.getBossbarY() : 12;
            this.drawTexturedModalRect(x, y, 0, 74, width, 5);
            this.drawTexturedModalRect(x, y, 0, 74, width, 5);

            if (l > 0) {
                this.drawTexturedModalRect(x, y, 0, 79, l, 5);
            }

            fontrenderer.drawStringWithShadow(bossName, selected ? x + width / 2f - (fontrenderer.getStringWidth(bossName) / 2f) : i / 2f - fontrenderer.getStringWidth(bossName) / 2f, y - 10f, 16777215);
            if (mc.currentScreen instanceof GuiChat && selected) {
                if (overlayModule.isBossbarHovered()) {
                    RenderUtil.drawOutlinedRect(overlayModule.getBossbarX() - 2, overlayModule.getBossbarY() - 10, overlayModule.getBossbarX() + width, overlayModule.getBossbarY() + 10, 2, new Color(0, 0, 0, 100).getRGB(), new Color(0, 0, 0, 160).getRGB());
                }
                else
                    RenderUtil.drawRect(overlayModule.getBossbarX() - 2, overlayModule.getBossbarY() - 10, overlayModule.getBossbarX() + width, overlayModule.getBossbarY() + 10, new Color(0, 0, 0, 50).getRGB());
            }
            glColor4f(1, 1, 1, 1);
            this.mc.getTextureManager().bindTexture(icons);
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
