...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void onDisconnect(IChatComponent reason)
    {
        ...
        {
            this.gameController.displayGuiScreen(new GuiDisconnected(new GuiMultiplayer(new StrifeMainMenu()), "disconnect.lost", reason));
        }
    }

    public void addToSendQueue(Packet packet) {
        PacketOutboundEvent packetOutboundEvent = new PacketOutboundEvent(packet);
        Client.INSTANCE.getEventBus().post(packetOutboundEvent);
        if(packetOutboundEvent.isCancelled()) return;
        this.netManager.sendPacket(packetOutboundEvent.getPacket());
    }
    ...
    public void handleConfirmTransaction(S32PacketConfirmTransaction packetIn)
    {
        ...
        {
//            if(DisablerModule.instance().toggled()) {
//                if(Minecraft.getMinecraft().thePlayer.positionUpdateTicks % 2 == 0) {
//                    this.addToSendQueue(new C0FPacketConfirmTransaction(packetIn.getWindowId(), (short) (packetIn.getActionNumber() - Minecraft.getMinecraft().thePlayer.positionUpdateTicks % 2), true));
//                }
//            } else {
