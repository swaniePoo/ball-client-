...
import me.dinozoid.strife.account.AccountStatus;
import me.dinozoid.strife.module.implementations.exploit.StaffAnalyzerModule;
...
//        if(ServerUtil.onServer()) {
//            Client.INSTANCE.getSocketClient().packetHandler().sendPacket(new CServerCommandPacket(CServerCommandPacket.CommandOperation.LIST_USERS, "", "capes"));
//        }

        if (!mc.isSingleplayer() && ServerUtil.onServer("Hypixel") && Client.INSTANCE.getAccountRepository().currentAccount() != null) {
            Client.INSTANCE.getAccountRepository().currentAccount().info().hypixelBanned(AccountStatus.UNBANNED);
            SessionInfoModule.instance().startPlayTime(System.currentTimeMillis());
            if (!Client.INSTANCE.getModuleRepository().moduleBy(StaffAnalyzerModule.class).recievedAPIKey())
                PacketUtil.sendPacketDelayedNoEvent(new C01PacketChatMessage("/api new"), 2000);
        }
