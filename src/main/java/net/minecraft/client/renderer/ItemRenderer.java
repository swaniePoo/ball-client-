...
import me.dinozoid.strife.module.implementations.visuals.AnimationsModule;
import me.dinozoid.strife.module.implementations.visuals.ChamsModule;
import me.dinozoid.strife.module.implementations.visuals.LowFireModule;
...
public class ItemRenderer {
    ...
    private void renderPlayerArm(AbstractClientPlayer clientPlayer, float equipProgress, float swingProgress) {
        if (chamsModule == null)
            chamsModule = Client.INSTANCE.getModuleRepository().moduleBy(ChamsModule.class);
        if (chamsModule.toggled() && chamsModule.handProperty().getValue() && chamsModule.coloredProperty().getValue()) {
            int color = RenderUtil.toColorRGB(chamsModule.getHandColor(2), chamsModule.handAlphaProperty().getValue().floatValue()).getRGB();
            glPushAttrib(GL_ALL_CLIENT_ATTRIB_BITS);
            glEnable(GL_BLEND);
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
            glDisable(GL_TEXTURE_2D);
            if (!chamsModule.materialProperty().getValue())
    ...
    private void transformFirstPersonItem(float equipProgress, float swingProgress) {
        ...
        if (AnimationsModule.instance().getAlwaysScale().getValue()) {
            GlStateManager.scale(AnimationsModule.instance().scaleProperty().getValue(),
                    AnimationsModule.instance().scaleProperty().getValue(),
                    AnimationsModule.instance().scaleProperty().getValue());
        }
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            GlStateManager.pushMatrix();
            boolean block = KillAuraModule.instance().target() != null && (Boolean) KillAuraModule.propertyRepository().propertyBy(KillAuraModule.class, "Autoblock").getValue();
            ...
            if (this.itemToRender != null) {
                if (this.itemToRender.getItem() instanceof ItemMap) {
                    ...
                    AnimationsModule animationsModule = AnimationsModule.instance();
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK:
                            ...
                            if (animationsModule.toggled()) {
                                GlStateManager.translate(
                                        animationsModule.itemXProperty().getValue(),
                                        animationsModule.itemYProperty().getValue(),
                                        animationsModule.itemZProperty().getValue());
                                switch (animationsModule.animationsModeProperty().getValue()) {
                                    case VANILLA: {
                                    ...
                                    break;
                                    case SWANK: {
                                        transformFirstPersonItem(f / 2, f1);
                                        GlStateManager.rotate(sqrtPI * 30.0F / 2.0F, -sqrtPI, -0.0F, 9.0F);
                                        GlStateManager.rotate(sqrtPI * 40.0F, 1.0F, -sqrtPI / 2.0F, -0.0F);
                                        doBlockTransformations();
                                    }
                                    break;
                                    case EXHIBITION: {
                                        GL11.glTranslated(0D, 0.1D, 0.0D);
                                        GL11.glTranslated(0D, sqrtPI * 0.05D, 0.0D);
                                        this.transformFirstPersonItem(f / 2, 0.0f);
                                        final double rotX = 0.0D, rotY = 0.1D, rotZ = 0.1D;
                                        GL11.glTranslated(rotX, rotY, rotZ);
                                        GlStateManager.rotate(-sqrtPI * 56 / 1.85F, sqrtPI / 4.8F, -2.0F, 145.0F);
                                        GlStateManager.rotate(-sqrtPI * 56, 1.0F, sqrtPI / 2.1F, 0.125F);
                                        GL11.glTranslated(-rotX, -rotY, -rotZ);
                                    ...
                                    break;
                                    case OLD: {
                                        transformFirstPersonItem(0, f1);
                                        doBlockTransformations();
                                        break;
                                    }
                                    case STELLA: {
                                        transformFirstPersonItem(0, 1);
                                        GlStateManager.rotate(sqrtPI * 45.0F, -sqrtPI, 0.3F, 0);
                                        doBlockTransformations();
                                        break;
                                    }
                                    case WAVE: {
                                        transformFirstPersonItem(f, 1);
                                        GlStateManager.rotate(sqrtPI * 45.0F, -sqrtPI, 0.3F, 0);
                                        doBlockTransformations();
                                        break;
                                    }
                                    case ETHEREAL: {
                                        transformFirstPersonItem(0, 1);
                                        GlStateManager.rotate(sqrtPI * 35, -0.1f, 0.1f, 0.1f);
                                        GlStateManager.rotate(-20, 1.0F, -0.5f, 1F);
                                        doBlockTransformations();
                                        break;
                                    }
                                    case DORTWARE: {
                                        transformFirstPersonItem(f / 2F, 0.0f);
                                    ...
                                    case KEKCLIENT: {
                                        transformFirstPersonItem(f, 0.0f);
                                        doBlockTransformations();
                                        GlStateManager.rotate(1.0f * -(Minecraft.getSystemTime() % 360L), 0.3f, 0.0f, 0.1f);
                                        break;
                                    }
                                    case LUNAR: {
                                        transformFirstPersonItem(0.15f, f1);
                                        doBlockTransformations();
                                        GlStateManager.translate(-0.5f, 0.2f, 0.0f);
                                        break;
                                    }
                                    case AVATAR: {
                                        GlStateManager.translate(0.56F, -0.52F, -0.71999997F);
                                        GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
                                        GlStateManager.rotate(piMulti * -20.0F, 0.0F, 1.0F, 0.0F);
                                        GlStateManager.rotate(sqrtPI * -20.0F, 0.0F, 0.0F, 1.0F);
                                        GlStateManager.rotate(sqrtPI * -40.0F, 1.0F, 0.0F, 0.0F);
                                        GlStateManager.scale(0.38F, 0.38F, 0.38F);
                                        doBlockTransformations();
                                        break;
                                    }
                                ...
                                if (!AnimationsModule.instance().getAlwaysScale().getValue()) {
                                    GlStateManager.scale(animationsModule.scaleProperty().getValue(),
                                            animationsModule.scaleProperty().getValue(),
                                            animationsModule.scaleProperty().getValue());
                                }
                            } else {
                                transformFirstPersonItem(f, 0.0F);
                    ...
                    if (AnimationsModule.instance().toggled()) {
                        switch (AnimationsModule.instance().hitModeProperty().getValue()) {
                            case NORMAL:
                                this.doItemUsedTransformations(f1);
                                break;
                            case SMOOTH:
                                this.doItemUsedTransformations(f);
                        }
    ...
    private void renderFireInFirstPerson(float partialTicks) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            float f8 = f7 + f;
            float f9 = -0.5F;
            GlStateManager.translate((float) (-(i * 2 - 1)) * 0.24F, Client.INSTANCE.getModuleRepository().moduleBy(LowFireModule.class).toggled() ? -0.55F : -0.3F, 0.0F);
