...
import me.dinozoid.strife.module.implementations.visuals.BlockOverlayModule;
...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener
{
    ...
    public void drawSelectionBox(EntityPlayer player, MovingObjectPosition movingObjectPositionIn, int execute, float partialTicks)
    {
        if (execute == 0 && movingObjectPositionIn.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
        {
            ...
            if (block.getMaterial() != Material.air && this.theWorld.getWorldBorder().contains(blockpos))
            {
                ...
                BlockOverlayModule blockOverlayModule = Client.INSTANCE.getModuleRepository().moduleBy(BlockOverlayModule.class);
                if(blockOverlayModule.toggled()) {
                    if (blockOverlayModule.getOutlineProperty().getValue())
                    ...
                    drawFilledSelectionBoundingBox(axisalignedbb.expand(0.0020000000949949026D, 0.0020000000949949026D, 0.0020000000949949026D).offset(-d0, -d1, -d2), new Color(OverlayModule.getColor(200)), 100);
                } else {
                    drawSelectionBoundingBox(axisalignedbb.expand(0.0020000000949949026D, 0.0020000000949949026D, 0.0020000000949949026D).offset(-d0, -d1, -d2));
                }
            }
    ...
    public static void drawFilledSelectionBoundingBox(AxisAlignedBB boundingBox, Color color, float alpha)
    {
        GL11.glBegin(GL11.GL_QUADS);
        RenderUtil.color(color, alpha);
