...
import me.dinozoid.strife.module.implementations.visuals.GlintColorizeModule;
...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    public void renderItem(ItemStack stack, IBakedModel model, int opacity)
    {
        if (stack != null)
        {
            GlStateManager.pushMatrix();
            ...
            if (model.isBuiltInRenderer())
            {
                GlStateManager.rotate(180.0F, 0.0F, 1.0F, 0.0F);
                GlStateManager.translate(-0.5F, -0.5F, -0.5F);
                GlStateManager.color(1.0F, 1.0F, 1.0F, opacity / 255f);
                GlStateManager.enableRescaleNormal();
                TileEntityItemStackRenderer.instance.renderByItem(stack);
            }
            else
            {
                ...
                this.renderModelHasEmissive = false;
                this.renderModel(model, new Color(255, 255, 255, opacity).getRGB(), stack);
    ...
    private void renderEffect(IBakedModel model)
    {
        if (!Config.isCustomItems() || CustomItems.isUseGlint())
        {
            if (!Config.isShaders() || !Shaders.isShadowPass)
            {
                ...
                this.renderModel(model, GlintColorizeModule.instance().toggled() ? OverlayModule.getColor(200) : -8372020);
                GlStateManager.popMatrix();
                GlStateManager.pushMatrix();
                GlStateManager.scale(8.0F, 8.0F, 8.0F);
    ...
    public void renderItemIntoGUI(ItemStack stack, float x, float y, int opacity)
    {
        this.renderItemGui = true;
        IBakedModel ibakedmodel = this.itemModelMesher.getItemModel(stack);
        GlStateManager.pushMatrix();
        ...
        this.renderItem(stack, ibakedmodel, opacity);
        GlStateManager.disableAlpha();
        GlStateManager.disableRescaleNormal();
        GlStateManager.disableLighting();
        GlStateManager.popMatrix();
    ...
    public void renderItemAndEffectIntoGUI(final ItemStack stack, float xPosition, float yPosition, int opacity)
    {
        if (stack != null && stack.getItem() != null)
        {
            this.zLevel += 50.0F;
            ...
            try
            {
                this.renderItemIntoGUI(stack, xPosition, yPosition, opacity);
            }
            catch (Throwable throwable)
            {
                CrashReport crashreport = CrashReport.makeCrashReport(throwable, "Rendering item");
