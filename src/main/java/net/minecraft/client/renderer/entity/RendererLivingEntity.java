...
import me.dinozoid.strife.module.implementations.visuals.OutlineESPModule;
import me.dinozoid.strife.util.player.PlayerUtil;
import me.dinozoid.strife.util.render.OutlineUtil;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    private OutlineESPModule outlineESPModule;
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z))) {
            ...
            if (chamsModule.toggled() && !chamsModule.coloredProperty().getValue()) {
            ...
            try {
                float f, f1;
                if (mc.thePlayer != null && entity == mc.thePlayer && mc.thePlayer.currentEvent != null) {
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float scaleFactor) {
        ...
        if (visible || visibleToPlayer) {
            if (!bindEntityTexture(entitylivingbaseIn)) return;
            ...
            if (outlineESPModule == null)
                outlineESPModule = Client.INSTANCE.getModuleRepository().moduleBy(OutlineESPModule.class);

            if (chamsModule == null)
                chamsModule = Client.INSTANCE.getModuleRepository().moduleBy(ChamsModule.class);

            if (outlineESPModule.toggled() && PlayerUtil.isValid(entitylivingbaseIn, outlineESPModule.targetsProperty())) {
                int color = outlineESPModule.getColor(2);
                glPushMatrix();
                GlStateManager.depthMask(true);
            ...
            GlowESPModule glowESPModule = Client.INSTANCE.getModuleRepository().moduleBy(GlowESPModule.class);
            if (chamsModule.toggled() && chamsModule.coloredProperty().getValue() && PlayerUtil.isValid(entitylivingbaseIn, chamsModule.targetsProperty())) {
                int color = RenderUtil.toColorRGB(chamsModule.getColor(2), chamsModule.alphaProperty().getValue().floatValue()).getRGB();
                int hiddenColor = RenderUtil.toColorRGB(chamsModule.getHiddenColor(2), chamsModule.alphaProperty().getValue().floatValue()).getRGB();
    ...
    protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures) {
        ...
        if (!flag && !flag1) {
            ...
            glTexEnv(GL_TEXTURE_ENV, GL_TEXTURE_ENV_COLOR, this.brightnessBuffer);
    ...
    public void renderName(T entity, double x, double y, double z) {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z))) {
            ...
            Client.INSTANCE.getEventBus().post(renderNametagEvent);
