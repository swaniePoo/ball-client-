...
public abstract class EntityLivingBase extends Entity {
    ...
    public int getArmSwingAnimationEnd() {
        if (AnimationsModule.instance().toggled() && AnimationsModule.instance().hitSlowdownProperty().getValue())
            return AnimationsModule.instance().hitSlowdownAmount().getValue().intValue();
    ...
    protected void jump() {
        ...
        Client.INSTANCE.getEventBus().post(playerJumpEvent);

        if(playerJumpEvent.getEntity() instanceof EntityPlayer && playerJumpEvent.isCancelled())
            return;
