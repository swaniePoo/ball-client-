...
import me.dinozoid.strife.module.implementations.combat.HitBoxModule;
import me.dinozoid.strife.module.implementations.movement.AutoJumpModule;
...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            double d4 = y;
            double d5 = z;
            boolean flag = this.onGround && this.isSneaking() && this instanceof EntityPlayer || (SafeWalkModule.instance().toggled() || ScaffoldModule.getInstance().doSafeWalk()) && this instanceof EntityPlayerSP;

            if (flag)
            {
                double d6;

                for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, -1.0D, 0.0D)).isEmpty(); entityX = x)
                {
                    if (x < d6 && x >= -d6)
                    {
                        x = 0.0D;
            ...
            final BlockStepEvent event = new BlockStepEvent(EventState.PRE, stepHeight);
            if (this instanceof EntityPlayerSP) {
                Client.INSTANCE.getEventBus().post(event);
                stepHeight = event.getHeight();
            }
            if (event.getHeight() > 0.0F && flag1 && (entityX != x || d5 != z)) {
                double stepX = x;
                double stepY = y;
                double stepZ = z;
                ...
                y = event.getHeight();
                List<AxisAlignedBB> list = this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().addCoord(entityX, y, d5));
                AxisAlignedBB axisalignedbb4 = this.getEntityBoundingBox();
                AxisAlignedBB axisalignedbb5 = axisalignedbb4.addCoord(entityX, 0.0D, d5);
                double yOffset = y;

                for (AxisAlignedBB axisalignedbb6 : list)
                {
                    yOffset = axisalignedbb6.calculateYOffset(axisalignedbb5, yOffset);
                }

                axisalignedbb4 = axisalignedbb4.offset(0.0D, yOffset, 0.0D);
                double xOffset = entityX;

                for (AxisAlignedBB axisalignedbb7 : list)
                {
                    xOffset = axisalignedbb7.calculateXOffset(axisalignedbb4, xOffset);
                }

                axisalignedbb4 = axisalignedbb4.offset(xOffset, 0.0D, 0.0D);
                double zOffset = d5;

                for (AxisAlignedBB axisalignedbb8 : list)
                {
                    zOffset = axisalignedbb8.calculateZOffset(axisalignedbb4, zOffset);
                }

                axisalignedbb4 = axisalignedbb4.offset(0.0D, 0.0D, zOffset);
                AxisAlignedBB offsetBB = this.getEntityBoundingBox();
                double yOffset2 = y;

                for (AxisAlignedBB axisalignedbb9 : list)
                {
                    yOffset2 = axisalignedbb9.calculateYOffset(offsetBB, yOffset2);
                }

                offsetBB = offsetBB.offset(0.0D, yOffset2, 0.0D);
                double xOffset2 = entityX;

                for (AxisAlignedBB axisalignedbb10 : list)
                {
                    xOffset2 = axisalignedbb10.calculateXOffset(offsetBB, xOffset2);
                }

                offsetBB = offsetBB.offset(xOffset2, 0.0D, 0.0D);
                double zOffset2 = d5;

                for (AxisAlignedBB axisalignedbb11 : list)
                {
                    zOffset2 = axisalignedbb11.calculateZOffset(offsetBB, zOffset2);
                }

                offsetBB = offsetBB.offset(0.0D, 0.0D, zOffset2);
                ...
                else
                {
                    ...
                    z = zOffset2;
                    y = -yOffset2;
                    this.setEntityBoundingBox(offsetBB);
                }
                ...
                if (stepX * stepX + stepZ * stepZ >= x * x + z * z)
                {
                    x = stepX;
                    y = stepY;
                    z = stepZ;
                    ...
                    event.setState(EventState.POST);
                    event.setHeight((float) (1 + y));
                    Client.INSTANCE.getEventBus().post(event);
                }
            }
            ...
            this.resetPositionToBB();
            this.isCollidedHorizontally = entityX != x || d5 != z;
            if(this == Minecraft.getMinecraft().thePlayer) {
                AutoJumpModule autoJumpModule = Client.INSTANCE.getModuleRepository().moduleBy(AutoJumpModule.class);
                autoJumpModule.x(x);
                autoJumpModule.z(z);
            }
    ...
    public void moveFlying(float strafe, float forward, float friction)
    {
        ...
        if (this instanceof EntityPlayerSP) {
            ...
            Client.INSTANCE.getEventBus().post(eventStrafe);
    ...
    public float getCollisionBorderSize() {
        return (HitBoxModule.instance().toggled()) ? HitBoxModule.instance().hitboxProperty().getValue().floatValue() : 0.1F;
    }
