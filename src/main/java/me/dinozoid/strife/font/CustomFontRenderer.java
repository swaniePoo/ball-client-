...
public final class CustomFontRenderer {
    ...
    public void drawString(final String text, final float x, final float y2, final int color) {
        renderString(RenderUtil.getText(text, false), x, y2, color, false);
    }

    public void drawCenteredString(final String text, final float x, final float y2, final int color) {
        final float width = getWidth(RenderUtil.getText(text, false)) / 2.0f;
        renderString(RenderUtil.getText(text, false), x - width, y2 - (getHeight(RenderUtil.getText(text, false)) / 2), color, false);
    }
    ...
    public void drawStringWithShadow(final String text, final float x, final float y2, final int color) {
        ...
        renderString(RenderUtil.getText(text, false), x, y2, color, true);
        GL11.glTranslated(-0.5, -0.5, 0.0);
        renderString(RenderUtil.getText(text, false), x, y2, color, false);
    }

    private void renderString(String text, float x, float y2, final int color, final boolean shadow) {
        if (text == "" || text.length() == 0) {
            return;
        }
        text = RenderUtil.getText(text, false);
        GL11.glPushMatrix();
        ...
        GlStateManager.color(c2.getRed() / multiplier, c2.getGreen() / multiplier, c2.getBlue() / multiplier, (color >> 24 & 255) / 255.0);
    ...
    public String trimStringToWidth(String text, float width, boolean reverse) {
        ...
        for (int k = i; k >= 0 && k < text.length() && f < width; k += j) {
            ...
            float f1 = getWidth(String.valueOf(c));
            if (flag) {
                flag = false;
                if (c != 108 && c != 76) {
                    if (c == 114 || c == 82) flag1 = false;
                } else flag1 = true;
            } else if (f1 < 0.0F) flag = true;
            else {
                f += f1;
                if (flag1) ++f;
            }
            if (f > width) break;
            if (reverse) stringbuilder.insert(0, c);
