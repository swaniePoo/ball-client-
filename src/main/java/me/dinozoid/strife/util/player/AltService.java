...
import me.dinozoid.strife.websocket.packet.implementations.CUserUpdatePacket;
import me.dinozoid.strife.websocket.packet.implementations.SUserUpdatePacket;
...
public class AltService extends MinecraftUtil {
    ...
    public static CompletableFuture<Boolean> login(final AlteningServiceType type, final String email, final String password) {
        ...
        Client.INSTANCE.getExecutorService().execute(() -> {
            ...
            try {
                ...
                Client.INSTANCE.getNotificationRepository().display(new Notification(Notification.NotificationType.SUCCESS, "Logged in", "Successfully logged in as " + mc.session.getUsername()));
//                Client.INSTANCE.getSocketClient().packetHandler().sendPacket(new CUserUpdatePacket(new SUserUpdatePacket.Value(SUserUpdatePacket.UpdateType.ACCOUNT_USERNAME, mc.session.getUsername())));
                toComplete.complete(true);
