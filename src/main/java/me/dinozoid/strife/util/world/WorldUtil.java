...
public final class WorldUtil extends MinecraftUtil {
    ...
    public static EntityLivingBase raycast(double range, float[] rotations, boolean behindWall) {
        ...
        Vec3 look = Entity.getVectorForRotation(rotations[1], rotations[0]);
        ...
        List<Entity> entities = mc.theWorld.getEntitiesInAABBexcluding(mc.thePlayer, mc.thePlayer.getEntityBoundingBox().addCoord(look.xCoord * range, look.yCoord * range, look.zCoord * range).expand(1, 1, 1), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity::canBeCollidedWith, EntityLivingBase.class::isInstance, (entity) -> PlayerUtil.isValid((EntityLivingBase) entity, KillAuraModule.instance().targetsProperty())));
        ...
        for (Entity ent : entities) {
            ...
            // check if player is inside mc.thePlayer's body
            if (axisalignedbb.isVecInside(eyes)) {
                if (range >= 0) {
                    raycastedEntity = entity;
                    range = 0;
                }
                // if found object and not in mc.thePlayer's body
            } else if (movingobjectposition != null) {

                if (behindWall) {
//                    MovingObjectPosition block = mc.theWorld.rayTraceBlocks(look, movingobjectposition.hitVec, false);
//                    if(block != null && block instanceof ) PlayerUtil.sendMessage(movingobjectposition.toString());
                }
        ...
        // set range to static variable out of method
