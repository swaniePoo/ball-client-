...
public abstract class Animation {
    ...
    public void setDirection(Direction direction) {
        if (this.direction != direction) {
            ...
            timerUtil.setCurrentMS(System.currentTimeMillis() - (duration - Math.min(duration, timerUtil.elapsed())));
        }
    }
    ...
    public double getOutput() {
        if (direction == Direction.FORWARDS) {
            ...
            if (isDone()) return 0;
            if (correctOutput()) {
                double revTime = Math.min(duration, Math.max(0, duration - timerUtil.elapsed()));
